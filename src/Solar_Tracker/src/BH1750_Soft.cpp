 ...
 */
BH1750_Soft::BH1750_Soft(byte addr) {

  BH1750_I2CADDR = addr;
  // Allows user to change SoftWire instance
  I2C = &SWire;
}
 ...
 */
bool BH1750_Soft::begin(Mode mode, byte addr, SoftWire* i2c) {
 ...
 *
 */
bool BH1750_Soft::measurementReady(bool maxWait) {
 ...
 */
float BH1750_Soft::readLightLevel() {
    ...
    int numBytes = I2C->requestFrom((int)BH1750_I2CADDR, (int)2);
    ...
    // Print MTreg factor if debug enabled
        ...
        if (BH1750_MODE == BH1750_Soft::ONE_TIME_HIGH_RES_MODE_2 ||
            BH1750_MODE == BH1750_Soft::CONTINUOUS_HIGH_RES_MODE_2) {
        level /= 2;
        }
